...
const seedWish: Wish = {
  ...
  createdAt: '2026-07-31T23:02:21.000Z',
};
...
export default function Guestbook() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (firestore) {
        await addDoc(collection(firestore, GUESTBOOK_COLLECTION), {
          name: wish.name,
      ...
      <div className="invite-shell">
        ...
        >
          ...
          <button
            ...
            className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#1a3460] py-3.5 text-sm tracking-wide text-[#f5efe0] transition-transform hover:scale-[1.01] disabled:opacity-70"
          ...
          {status === 'error' && (
            <p className="mt-3 text-center text-sm text-[#a3392b]" data-testid="text-guestbook-error">
              Please add your name and wishes.
            </p>
          )}
          {status === 'success' && (
            <p
